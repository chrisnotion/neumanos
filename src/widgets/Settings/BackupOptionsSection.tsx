...
export const BackupOptionsSection: React.FC<BackupOptionsSectionProps> = ({
  ...
  const toggleCompression = async () => {
    ...
    onRefresh();
    onMessage({ type: 'info', text: `压缩功能已${newValue ? '开启' : '关闭'}` });
  ...
  const toggleReminder = async () => {
    ...
    onRefresh();
    onMessage({ type: 'info', text: `备份周期提醒已${newValue ? '启用' : '停用'}` });
    ...
    <div className="bento-card p-6">
      ...
      <div className="space-y-4">
        ...
        <label className="flex items-center justify-between p-4 bg-surface-light-elevated dark:bg-surface-dark-elevated rounded-lg cursor-pointer hover:bg-border-light dark:hover:bg-border-dark transition-colors">
          <div>
            <p className="font-medium text-text-light-primary dark:text-text-dark-primary">启用 Gzip 高效压缩</p>
        ...
        <label className="flex items-center justify-between p-4 bg-surface-light-elevated dark:bg-surface-dark-elevated rounded-lg cursor-pointer hover:bg-border-light dark:hover:bg-border-dark transition-colors">
          <div>
            <p className="font-medium text-text-light-primary dark:text-text-dark-primary">定期备份提醒</p>
            <p className="text-sm text-text-light-secondary dark:text-text-dark-secondary">
              每隔 {preferences.reminderDays} 天弹出备份提醒通知
            </p>
          </div>
          <div className="flex items-center gap-2">
