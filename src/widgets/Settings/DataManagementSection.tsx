...
const MODULE_CONFIG: Record<string, { label: string; icon: React.FC<{ className?: string }> }> = {
  'notes-storage': { label: '灵感笔记', icon: FileText },
  'kanban-tasks': { label: '任务看板', icon: CheckSquare },
  'calendar-events': { label: '日程日历', icon: Calendar },
  'time-tracking': { label: '工时追踪', icon: Clock },
  'settings-storage': { label: '系统设置', icon: Database },
  'theme-storage': { label: '外观主题', icon: Database },
  'habits-storage': { label: '习惯例程', icon: BarChart3 },
};
...
export const DataManagementSection: React.FC<DataManagementSectionProps> = ({
  ...
  const handleExportAll = async () => {
    try {
      ...
      onMessage({
        type: 'success',
        text: `全量数据导出成功！文件：${result.filename}（${formatFileSize(result.size)}）`,
      });
    } catch (error) {
      onMessage({ type: 'error', text: `数据导出失败：${error}` });
    } finally {
  ...
  const handleClearAll = async () => {
    ...
    try {
      ...
      onMessage({
        type: 'success',
        text: '全量本地数据已清空。页面将于 2 秒后自动重新载入。',
      });

      setTimeout(() => window.location.reload(), 2000);
    } catch (error) {
      onMessage({ type: 'error', text: `清空数据失败：${error}` });
    } finally {
      setIsClearing(false);
    ...
    <div className="bento-card p-6">
      ...
      <div className="flex items-center gap-3 mb-6 p-4 bg-surface-light-elevated dark:bg-surface-dark-elevated rounded-lg">
        ...
        <div className="text-right">
          ...
          <p className="text-sm text-text-light-secondary dark:text-text-dark-secondary">
            共 {modules.reduce((sum, m) => sum + m.itemCount, 0)} 条业务数据
          </p>
        </div>
      </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-text-light-secondary dark:text-text-dark-secondary ml-2 flex-shrink-0">
                      ...
                      {mod.itemCount > 0 && ` (${mod.itemCount} 项)`}
                    </span>
                  </div>
                  <div className="w-full bg-border-light dark:border-border-dark rounded-full h-1.5 overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-text-light-secondary dark:text-text-dark-secondary mb-4">
                此操作将永久彻底抹除您本地保存的所有笔记、任务、日程事件、工时打卡记录及偏好设置。
                此过程完全不可逆，强烈建议在清空前先导出完整的本地备份文件。
              </p>

              <p className="text-sm font-medium text-text-light-primary dark:text-text-dark-primary mb-2">
                请输入 <span className="font-mono text-accent-red">DELETE ALL DATA</span> 以确认执行清空：
              </p>
