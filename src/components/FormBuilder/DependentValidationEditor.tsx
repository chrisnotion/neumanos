...
export function DependentValidationEditor({
  ...
  const handleAddValidation = () => {
    if (availableFields.length === 0) {
      toast.warning('没有可用于条件的字段', '请先在当前字段上方添加字段。');
      return;
    }
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  >
                    <option value="require_if">当条件满足时必填...</option>
                    <option value="min_if">当条件满足时限制最小值...</option>
                    <option value="max_if">当条件满足时限制最大值...</option>
                    <option value="pattern_if">当条件满足时正则匹配...</option>
                  </select>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value="is_answered">已填写/已选择</option>
                        <option value="is_not_answered">未填写/未选择</option>
                      </select>
                    </div>
...
function getValidationSummary(validation: DependentValidation, triggerFieldLabel: string): string {
  ...
  const validationType =
    type === 'require_if'
      ? '必填'
      : type === 'min_if'
      ? `最小值需为: ${validationRule.value}`
      : type === 'max_if'
      ? `最大值需为: ${validationRule.value}`
      : `需匹配正则: ${validationRule.value}`;

  return `当 [${triggerFieldLabel}] ${operatorText}${valueText} 时，当前字段 ${validationType}`;
}
