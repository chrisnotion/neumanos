...
export function WeeklyTimesheetView() {
  ...
  const formatWeekRange = () => {
    ...
    return `${startYear}年${startMonth}月${start.getDate()}日 - ${endYear}年${endMonth}月${end.getDate()}日`;
  };
    ...
    <div className="space-y-4">
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-4 py-8 text-center text-text-light-secondary dark:text-text-dark-secondary">
                    未发现项目工时记录。可在「项目归属」栏目中创建项目以开始使用周工时单。
                  </td>
                </tr>
              ) : (
                rows.map((row) => (
                        ...
                        >
                          {isEditing ? (
                            ...
                            <button
                              ...
                              title={hasTime ? `${cell.entries.length} 条记录` : '点击补录工时'}
                            >
                              {hasTime ? (
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="p-4 bg-surface-light dark:bg-surface-dark-elevated rounded-lg border border-border-light dark:border-border-dark">
          <div className="text-sm text-text-light-secondary dark:text-text-dark-secondary">周总时长</div>
          <div className="text-2xl font-mono font-bold text-accent-primary mt-1">
            {formatDuration(grandTotal, { showSeconds: false })}
          </div>
        </div>
        <div className="p-4 bg-surface-light dark:bg-surface-dark-elevated rounded-lg border border-border-light dark:border-border-dark">
          <div className="text-sm text-text-light-secondary dark:text-text-dark-secondary">活跃项目数</div>
