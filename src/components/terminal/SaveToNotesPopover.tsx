...
export const SaveToNotesPopover: React.FC<SaveToNotesPopoverProps> = ({
  ...
  const handleSaveToDailyNote = useCallback(async () => {
    ...
    try {
      ...
      if (saveResult.skipped) {
        toast.info(`已保存至“${note.title}”`);
      } else {
        toast.success(`已保存至“${note.title}”`);
      }
      onSaveComplete();
    } catch (error) {
      toast.error('保存至今日笔记失败');
  ...
  const handleSaveToRecentNote = useCallback(async (noteId: string, noteTitle: string) => {
    ...
    try {
      ...
      if (result.skipped) {
        toast.info(`已保存至“${noteTitle}”`);
      } else {
        toast.success(`已保存至“${noteTitle}”`);
      }
      onSaveComplete();
    } catch (error) {
      toast.error('保存至笔记失败');
  ...
  const handleCreateNewNote = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`已创建并保存至“${note.title}”`);
      onSaveComplete();
    } catch (error) {
      toast.error('创建笔记失败');
